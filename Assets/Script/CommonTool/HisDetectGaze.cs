...
{
    public static void Proud<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class HisDetectGaze : RomeEndocrine<HisDetectGaze>
{
    // Start is called before the first frame update
    public bool DeftImpose;
    public bool SoTune;

    static public HisDetectGaze instance;
    ...
    /// <returns></returns>
    public Vector3 DigGuideSpend(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject HisDraperyWhenScript(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float SpendUpStock(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Stock180Up360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int HisHorseOurselves(Vector2 p1, Vector2 p2)
    {
        float angle = Stock180Up360(SpendUpStock(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 DigWorshipUserFlat(Renderer Renderer)
    {
    ...
    public Vector2 DigWorshipUserFlat(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 DigSilentFlat(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 DigLearnMinstrel()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = DigConferGesso();
        float screenH = DigConferCooper();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int DigComplyFrown(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] HisLuxuryLiberate(int total, int count)
    {
    ...
    public Dictionary<string, object> HuddleUpMechanical(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string MechanicalUpHuddle(Dictionary<string, object> Info)
    {
    ...
    public int YamPollCity(string dateStart, DateTime dateEnd)
    {
    ...
    public int TugPollCity(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan PollCity(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 HisBurOrTear(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = HisBurOrTear(canvas, text, strFragmentIndex);
            Vector3 IronBur= HisBurOrTear(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + IronBur) * 0.5f;
        }
        else
        {
            stringPos = HisBurOrTear(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 HisBurOrTear(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float BlackoutByDose(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Wary= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string BoulderDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string KinsmanDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Wary;
