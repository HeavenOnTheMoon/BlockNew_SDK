...
public class HorseLegend : MonoBehaviour
{
    /// <summary>
    /// 查找子节点对象
    /// 内部使用递归
    ...
    /// <returns></returns>
    public static Transform TollUSAVarveSalt(GameObject goParent,string childName)
    {
        //查找结果
        ...
        if (searchTrans == null)
        {
            ...
            {
                searchTrans = TollUSAVarveSalt(trans.gameObject, childName);
                if (searchTrans != null)
                {
                    return searchTrans;
                }
    ...
    /// <returns></returns>
    public static T HisUSAVarveSaltKilometerCuticle<T>(GameObject goParent,string childName) where T:Component
    {
        //查找特定子节点
        Transform searchTranformNode = null;
        searchTranformNode = TollUSAVarveSalt(goParent, childName);
    ...
    /// <param name="childName">子对象名称</param>
    /// <returns></returns>
    public static T MobVarveSaltKilometer<T>(GameObject goParent,string childName) where T : Component
    {
        Transform searchTransform = null;
        searchTransform = TollUSAVarveSalt(goParent, childName);
    ...
    public static void MobVarveSaltUpDeadlySalt(Transform parents,Transform child)
    {
