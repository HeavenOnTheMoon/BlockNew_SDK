...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<WitComaEka>()[0].AlgaRye != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
#if UNITY_ANDROID
        if (AppLovinSettings.Instance.desKey != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("max key");
        }
       ...
       /* if (Resources.FindObjectsOfTypeAll<ADManager>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        ...
        if (Resources.FindObjectsOfTypeAll<WitComaEka>()[0].WhenRosy != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
...
#if UNITY_ANDROID
        AppLovinSettings.Instance.desKey = info.Applovin_SDK_KEY;
#else
       ...
       /* UnityEditorInternal.ComponentUtility.CopyComponent(GameObject.Find("Adjust").GetComponent<com.adjust.sdk.Adjust>());
        ...
        if (!GameObject.Find("MainManager").GetComponent<BuzzIDFaculty>())
        {
            GameObject.Find("MainManager").AddComponent<BuzzIDFaculty>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<BuzzIDFaculty>().Inner = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<BuzzIDFaculty>().appid = info.PackageName;
#endif

        Resources.FindObjectsOfTypeAll<WitComaEka>()[0].WhenRosy = info.GameCode;
        Resources.FindObjectsOfTypeAll<WitComaEka>()[0].AlgaRye = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<WitComaEka>()[0].AlgaDustyRye = info.BaseUrl + CChoppy.DustyUrl;
        Resources.FindObjectsOfTypeAll<WitComaEka>()[0].AlgaChoppyRye = info.BaseUrl + CChoppy.ChoppyRye;
        Resources.FindObjectsOfTypeAll<WitComaEka>()[0].AlgaPortRye = info.BaseUrl + CChoppy.PortRye;
        Resources.FindObjectsOfTypeAll<WitComaEka>()[0].AlgaPlentyRye = info.BaseUrl + CChoppy.PlentyRye;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CChoppy.ChoppyRye + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CChoppy.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CChoppy.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif

        WitArabFaculty.HisEquality().RootHis(url,
       (data) => {
